...
public class Debtor
{
    ...
    public Debtor(string fullName, DateTime birthDate, string phone, string email, string address, int debt)
    {
        FullName = fullName;
        BirthDate = birthDate;
        Phone = phone;
...
public class Program
{
    static void Main()
    {
        ...
        var rhytaOrDayrepDebtors = debtors.Where(d => d.Email.Contains("rhyta.com") || d.Email.Contains("dayrep.com")).ToList();
        Console.WriteLine("2) Debtors with rhyta.com or dayrep.com email:");
        PrintDebtors(rhytaOrDayrepDebtors);

        var age26To36Debtors = debtors.Where(d => {
            var age = DateTime.Now.Year - d.BirthDate.Year;
            return age >= 26 && age <= 36;
        }).ToList();
        Console.WriteLine("\n3) Debtors aged between 26 and 36:");
        PrintDebtors(age26To36Debtors);

        var debtLessThan5000Debtors = debtors.Where(d => d.Debt <= 5000).ToList();
        Console.WriteLine("\n4) Debtors with debt less than 5000:");
        PrintDebtors(debtLessThan5000Debtors);

        var longNameAndMany7Debtors = debtors.Where(d => d.FullName.Length > 18 && d.Phone.Count(c => c == '7') >= 2).ToList();
        Console.WriteLine("\n5) Debtors with full name longer than 18 characters and phone number containing at least two 7s:");
        PrintDebtors(longNameAndMany7Debtors);

        var winterBornDebtors = debtors.Where(d => d.BirthDate.Month == 12 || d.BirthDate.Month == 1 || d.BirthDate.Month == 2).ToList();
        Console.WriteLine("\n7) Debtors born in winter:");
        PrintDebtors(winterBornDebtors);
        ...
        var aboveAverageDebtDebtors = debtors.Where(d => d.Debt > averageDebt)
                                             ...
                                             .ToList();
        Console.WriteLine("\n8) Debtors with debt above average, sorted by last name and then by debt descending:");
        PrintDebtors(aboveAverageDebtDebtors);

        var no8InPhoneDebtors = debtors.Where(d => !d.Phone.Contains('8')).Select(d => new {
            LastName = d.FullName.Split().Last(),
            Age = DateTime.Now.Year - d.BirthDate.Year,
            Debt = d.Debt
        }).ToList();
        Console.WriteLine("\n9) Debtors without an 8 in their phone number:");
        foreach (var d in no8InPhoneDebtors)
        {
            Console.WriteLine($"{d.LastName}, {d.Age}, {d.Debt}");
        }

        var threeSameLetterDebtors = debtors.Where(d => {
            var name = d.FullName.Replace(" ", "").ToLower();
            return name.GroupBy(c => c).Any(g => g.Count() >= 3);
        }).OrderBy(d => d.FullName).ToList();
        Console.WriteLine("\n11) Debtors with at least 3 identical letters in name:");
        PrintDebtors(threeSameLetterDebtors);

        var mostCommonBirthYear = debtors.GroupBy(d => d.BirthDate.Year)
                                         .OrderByDescending(g => g.Count())
                                         .First()
                                         .Key;
        Console.WriteLine($"\n13) Year with most debtors born: {mostCommonBirthYear}");

        var top5DebtDebtors = debtors.OrderByDescending(d => d.Debt).Take(5).ToList();
        Console.WriteLine("\n14) Top 5 debtors with highest debt:");
        PrintDebtors(top5DebtDebtors);

        var totalDebt = debtors.Sum(d => d.Debt);
        Console.WriteLine($"\n15) Total debt of all debtors: {totalDebt}");

        var ww2Debtors = debtors.Where(d => d.BirthDate.Year <= 1945).ToList();
        Console.WriteLine("\n16) Debtors who experienced WWII:");
        PrintDebtors(ww2Debtors);

        var noRepeatingDigitsDebtors = debtors.Where(d => d.Phone.Distinct().Count() == d.Phone.Length).Select(d => new {
            d.FullName,
            d.Debt
        }).ToList();
        Console.WriteLine("\n18) Debtors without repeating digits in phone number:");
        foreach (var d in noRepeatingDigitsDebtors)
        {
            Console.WriteLine($"{d.FullName}, {d.Debt}");
        }

        var canRepayByBirthdayDebtors = debtors.Where(d => {
            var monthsToBirthday = (d.BirthDate.Month - DateTime.Now.Month + 12) % 12;
            var totalPayments = monthsToBirthday * 500;
            return totalPayments >= d.Debt;
        }).ToList();
        Console.WriteLine("\n19) Debtors who can repay their debt by their next birthday with monthly 500 AZN payments:");
        PrintDebtors(canRepayByBirthdayDebtors);

        var smileDebtors = debtors.Where(d => {
            var name = d.FullName.ToLower();
            return "smile".All(c => name.Contains(c));
        }).ToList();
        Console.WriteLine("\n20) Debtors whose names can form the word 'smile':");
        PrintDebtors(smileDebtors);
    }

    static void PrintDebtors(List<Debtor> debtors)
    {
        foreach (var debtor in debtors)
        {
            Console.WriteLine($"{debtor.FullName}, {debtor.BirthDate.ToShortDateString()}, {debtor.Phone}, {debtor.Email}, {debtor.Address}, {debtor.Debt}");
        }
    }
}
